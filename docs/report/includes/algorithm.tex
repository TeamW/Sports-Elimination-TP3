...
\begin{algorithm}[H]
  ...
  \SetKwData{Sink}{sink} \SetKwData{T}{team} \SetKwData{Flow}{flow}
  ...
  \KwResult{A boolean value representing the elimination status of team $\T$}
  \Begin{
      $Establish \Graph$\;
      \For{Edge in graph}{Set flow of AdjListNode to zero}
      \maxFlow of \Graph\leftarrow\capacity of each \Edge from \Source\;
      \If{\T is trivially eliminated}{\emph{return eliminated}
      ...
      \If{\capacity == 0(saturating Maximum Flow)}{
        $\T$.setEliminated\leftarrow not eliminated\;
        \emph{return not eliminated}\;
      }
      \Else{
        \Cert\leftarrow team nodes reachable from \Source\;
        \T.setEliminated\leftarrow eliminated\;
