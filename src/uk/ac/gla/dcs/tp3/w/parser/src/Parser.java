...
public class Parser{
	...
	public static void main(String[] args){
		//ArrayList<League> leagueEachDay = new ArrayList<League>();			//array of days in season each with seperate state
		...
		try {
			...
			//leagueEachDay.add(new League());				//date emulation (adding a day and manipulating it's state below)
			while(fileScanner.hasNextLine() || thisLine.isEmpty()){
				...
				for(int i = 6; i < thisLine.length(); i++){			//loop through characters in the line starting at first team name
				    ...
				    if(hyphenCheck && (c != '0' && c != '1' && c != '2' && c != '3' && c != '4' && c != '5' && c != '6' && c != '7' && c != '8' && c != '9') && c != '\t' && c != ':' && c != '$'){
				    	team2Name += c;
				    }
				...
				Team t1 = new Team(removeSpaces(team1Name));	//creating 2 temporary team objects to store parsed info
				Team t2 = new Team(removeSpaces(team2Name));	//handle null pointer error
